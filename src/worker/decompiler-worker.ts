...
import {
  smartRecoverFunctionsFromPE,
  type SmartRecoveredFunction,
} from '../pe-runtime-functions.js'
...
function parseJavaExceptionSummary(text: string | undefined):
  ...
  const match = exceptionLine.match(
    /((?:java|javax|ghidra)\.[A-Za-z0-9_.$]+(?:Exception|Error))(?::\s*(.*))?/
  )
  if (!match) {
    return undefined
  }
...
export function normalizeGhidraError(
  ...
  const evidence = [
    ...
    diagnostics?.java_exception
      ...
      : '',
    truncateDiagnosticText(diagnostics?.stderr)
      ? `stderr=${truncateDiagnosticText(diagnostics?.stderr)}`
      : '',
    truncateDiagnosticText(diagnostics?.stdout)
      ? `stdout=${truncateDiagnosticText(diagnostics?.stdout)}`
      : '',
  ].filter((item): item is string => item.length > 0)

  const withStage = (summary: string): string => (stage ? `${stage}: ${summary}` : summary)
  ...
  if (/unable to lock project|lockexception/i.test(corpus)) {
    return {
      ...
      stage,
      summary: withStage(
        'Ghidra project lock prevented the script from acquiring the project workspace.'
      ),
      remediation_hints: [
  ...
  if (
    /Directory not found:|FileNotFoundException: Directory not found|DefaultProjectManager\.createProject/i.test(
      corpus
    )
  ) {
    return {
      ...
      stage,
      summary: withStage(
        'Ghidra could not create or open the project because the project directory is missing or invalid.'
      ),
      remediation_hints: [
  ...
  if (
    /JAVA_HOME|UnsupportedClassVersionError|class file version|JavaRuntime|java version/i.test(
      corpus
    )
  ) {
    return {
      ...
      stage,
      summary: withStage(
        'Ghidra launch appears to be blocked by a missing or incompatible Java runtime.'
      ),
      remediation_hints: [
  ...
  if (/spawn.*EINVAL/i.test(corpus) || /EINVAL/i.test(diagnostics?.spawn_error || '')) {
    return {
      ...
      stage,
      summary: withStage(
        'Ghidra process could not be spawned due to Windows batch/script invocation mismatch (EINVAL).'
      ),
      remediation_hints: [
  ...
  if (/Ghidra was not started with PyGhidra|Python is not available/i.test(corpus)) {
    return {
      ...
      stage,
      summary: withStage(
        'PyGhidra is unavailable in the active environment, so Python post-scripts cannot run.'
      ),
      remediation_hints: [
  ...
  if (
    diagnostics?.spawn_error ||
    (error instanceof GhidraProcessError && error.errorCode === 'E_SPAWN')
  ) {
    return {
      code: 'spawn_failure',
      category: 'environment',
...
export class DecompilerWorker {
  ...
  private async resolveSamplePathForSample(sampleId: string): Promise<string> {
    const { samplePath, integrity } = await resolvePrimarySamplePath(
  ...
  private reportProgress(
    options: GhidraOptions | undefined,
    progress: number,
    stage: string,
    detail?: string
  ): void {
  ...
  private buildGhidraRuntimeLogPath(sampleId: string, stage: string, projectKey?: string): string {
    return this.buildGhidraCommandLogPath(sampleId, stage, projectKey).replace(
      /\.command\.log$/i,
      '.ghidra.log'
    )
  }
  ...
  private findLatestProjectInGhidraDir(
    ghidraDir: string
  ): { projectPath: string; projectKey: string } | null {
   ...
   */
  private spawnGhidraProcess(invocation: ProcessInvocation, cwd: string): ChildProcess {
  ...
  private async executeMainAnalysis(
    ...
    const ghidraRuntimeLogPath = this.buildGhidraRuntimeLogPath(
      sampleId,
      'analyze_main',
      projectKey
    )
  ...
  private async tryExtractFunctionsWithFallback(
    ...
    const runAndParse = async (scriptName: string): Promise<GhidraAnalysisOutput | undefined> => {
      ...
      try {
        ...
        const diagnostics =
          parseError instanceof GhidraOutputParseError ? parseError.diagnostics : output.diagnostics
        attempts.push({
          script: scriptName,
          stdout: output.stdout,
          stderr: output.stderr,
  ...
  private buildCapabilityReadyStatus(target: string, warnings?: string[]): GhidraCapabilityStatus {
    return {
      available: true,
      status: 'ready',
      target,
  ...
  private resolveGhidraAnalysisForCapability(
    ...
    projectPath: string
    projectKey: string
    readiness: GhidraCapabilityStatus
  } {
  ...
  async analyze(sampleId: string, options: GhidraOptions = {}): Promise<AnalysisResult> {
    ...
    try {
      ...
      const { projectPath, projectKey } = createGhidraProject(ghidraProjectRoot, options.projectKey)
      ...
      trackProgress(
        ...
        extraction.scriptUsed || 'fallback_completed'
      )
      ...
      const status: 'done' | 'partial_success' = analysisOutputHasFunctions
        ? 'done'
        : 'partial_success'
        ...
        new Set(
          [
            ...
            ...extractionAttempts.map(
              (attempt) => attempt.command_log_path || attempt.diagnostics?.log_path
            ),
  ...
  async rankFunctions(
    ...
    const dynamicResolverAPIs = new Set([
      'GetProcAddress',
      'LoadLibrary',
      'LoadLibraryA',
      'LoadLibraryW',
    ])
    const dynamicResolverLookup = new Set(
      Array.from(dynamicResolverAPIs, (item) => item.toLowerCase())
    )
      ...
      target: {
        ...
        resolved_address:
          typeof targetRecord.resolved_address === 'string'
            ...
            : undefined,
        resolved_name:
          typeof targetRecord.resolved_name === 'string' ? targetRecord.resolved_name : undefined,
      },
      ...
      for (const rawMatch of parsed.matches) {
        ...
        const matchTypes = Array.isArray(match.match_types)
          ? match.match_types.filter(
              (item): item is 'api_call' | 'string_reference' | 'api_call_index' =>
    ...
    const dbFunctions = functions.map((func) => {
      ...
      return {
        ...
        caller_count: Math.max(
          func.caller_count,
          func.callers.length,
          func.caller_relationships?.length || 0
        ),
        callee_count: Math.max(
          func.callee_count,
          func.callees.length,
          func.callee_relationships?.length || 0
        ),
