 ...
 */
export const peImportsExtractToolDefinition: ToolDefinition = {
  name: TOOL_NAME,
 ...
 * Requirements: 3.1, 3.2
 */
export function createPEImportsExtractHandler(deps: PluginToolDeps) {
  const { workspaceManager, database, cacheManager } = deps
  return async (args: ToolArgs): Promise<WorkerResult> => {
    const input = args as PEImportsExtractInput
