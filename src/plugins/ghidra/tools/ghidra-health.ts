...
export const ghidraHealthOutputSchema = z.object({
  ...
  data: z
    .object({
      ...
      required_user_inputs: z.array(z.any()),
    })
    .optional(),
...
interface GhidraHealthDependencies {
  checkGhidra?: (timeoutMs: number) => any
  decompilerWorker?: Pick<any, 'decompileFunction' | 'getFunctionCFG'>
}
...
function selectProbeAnalysis(
  ...
  sampleId?: string
): { sampleId: string; analysis: any } | null {
  if (!database) {
    return null
  }
...
export function createGhidraHealthHandler(
  ...
  return async (args: unknown): Promise<ToolResult> => {
    try {
      ...
      const probeSelection = selectProbeAnalysis(database, findBestGhidraAnalysis, input.sample_id)
