...
```powershell
# Windows PowerShell
Copy-Item "C:\path\to\sample.exe" "D:\Docker\decompile-mcp-server\samples\sample.exe"
```

或者使用 `docker cp`：

```powershell
docker cp "C:\path\to\sample.exe" windows-exe-decompiler-mcp:/samples/sample.exe
```
...
module.exports = {
  ...
  execute: async (params) => {
    ...
    const samplesDir = 'D:\\Docker\\decompile-mcp-server\\samples';
...
```powershell
# 复制样本到挂载目录
Copy-Item "C:\malware\suspicious.exe" "D:\Docker\decompile-mcp-server\samples\suspicious.exe"
```
...
```powershell
# 检查目录权限
Get-Acl "D:\Docker\decompile-mcp-server\samples" | Format-List
```
...
```powershell
docker inspect windows-exe-decompiler-mcp --format '{{ json .Mounts }}' | jq
```
