/**
 * Unit tests for code.module.review.prepare tool
 */
...
describe('code.module.review.prepare tool', () => {
  ...
  describe('Input validation', () => {
    test('should accept valid input', () => {
      const result = codeModuleReviewPrepareInputSchema.safeParse({ sample_id: 'sha256:abc123def456' })
      expect(result.success).toBe(true)
    })

    test('should reject empty input', () => {
      const result = codeModuleReviewPrepareInputSchema.safeParse({})
      expect(result.success).toBe(false)
    })

    test('should reject invalid types', () => {
      const result = codeModuleReviewPrepareInputSchema.safeParse({ sample_id: 123 })
      expect(result.success).toBe(false)
    })
  })
  ...
  describe('Handler', () => {
    test('should return error for non-existent resource', async () => {
      const handler = createCodeModuleReviewPrepareHandler(mockWorkspaceManager, mockDatabase, mockCacheManager)
