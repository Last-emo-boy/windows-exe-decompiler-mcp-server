/**
 * Unit tests for code.function.rename.apply tool
 */
...
describe('code.function.rename.apply tool', () => {
  ...
  describe('Input validation', () => {
    ...
    test('should reject empty input', () => {
      const result = codeFunctionRenameApplyInputSchema.safeParse({})
      expect(result.success).toBe(false)
    })

    test('should reject invalid types', () => {
      const result = codeFunctionRenameApplyInputSchema.safeParse({ sample_id: 123 })
      expect(result.success).toBe(false)
    })
  })
  ...
  describe('Handler', () => {
    test('should return error for non-existent resource', async () => {
      const handler = createCodeFunctionRenameApplyHandler(mockWorkspaceManager, mockDatabase)
