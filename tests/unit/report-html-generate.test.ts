...
import { createReportHtmlGenerateHandler, ReportHtmlGenerateInputSchema } from '../../src/plugins/visualization/tools/report-html-generate.js'
...
describe('report.html.generate tool', () => {
  ...
  describe('Input validation', () => {
    test('should accept valid input', () => {
      const result = ReportHtmlGenerateInputSchema.safeParse({ sample_id: 'sha256:abc123def456' })
  ...
  describe('Handler', () => {
    test('should return error for non-existent resource', async () => {
      const handler = createReportHtmlGenerateHandler({ workspaceManager: mockWorkspaceManager, database: mockDatabase } as any)
