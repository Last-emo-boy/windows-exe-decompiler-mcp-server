/**
 * Unit tests for code.functions.search tool
 */
...
describe('code.functions.search tool', () => {
  ...
  describe('Input validation', () => {
    ...
    test('should reject empty input', () => {
      const result = codeFunctionsSearchInputSchema.safeParse({})
      expect(result.success).toBe(false)
    })

    test('should reject invalid types', () => {
      const result = codeFunctionsSearchInputSchema.safeParse({ sample_id: 123 })
      expect(result.success).toBe(false)
    })
  })
  ...
  describe('Handler', () => {
    test('should return error for non-existent resource', async () => {
      const handler = createCodeFunctionsSearchHandler(mockWorkspaceManager, mockDatabase)
