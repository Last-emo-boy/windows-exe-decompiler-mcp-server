/**
 * Unit tests for code.module.review.apply tool
 */
...
describe('code.module.review.apply tool', () => {
  ...
  describe('Input validation', () => {
    ...
    test('should reject empty input', () => {
      const result = codeModuleReviewApplyInputSchema.safeParse({})
      expect(result.success).toBe(false)
    })

    test('should reject invalid types', () => {
      const result = codeModuleReviewApplyInputSchema.safeParse({ sample_id: 123 })
      expect(result.success).toBe(false)
    })
  })
  ...
  describe('Handler', () => {
    test('should return error for non-existent resource', async () => {
      const handler = createCodeModuleReviewApplyHandler(mockWorkspaceManager, mockDatabase)
